...
class HavannahGUI:
    ...
    def __init__(
        ...
        self.fuzz_clicks = 0
        self.fuzz_valid_clicks = 0
        self.fuzz_occupied_clicks = 0
        self.fuzz_redraws = 0
        self.fuzz_status_updates = 0
        self.fuzz_resets = 0

        self._bug_skip_next_canvas_clear = False
        self._bug_keep_stale_board_label = False
    ...
    def _build_layout(self):
        ...
        tk.Label(
            self.top_bar,
            textvariable=self.board_label_var,
            anchor="e",
        ...
        self.info_var.set(
            "Click su un esagono per giocare. Bianco inizia. "
            "Vittoria con bridge, fork o ring."
        )
    ...
    def _ask_board_size(self, initial=False):
        ...
        selected_size = tk.IntVar(
            value=self.board_size if self.board_size is not None else 8
        )
        ...
        tk.Label(
            panel,
            text="La partita iniziera' subito dopo la selezione.",
            justify=tk.LEFT,
            pady=8,
        ).pack(anchor="w")
    ...
    def _start_session(self, board_size):
        ...
        if self.enable_injected_bugs and self._bug_keep_stale_board_label:
            self._bug_keep_stale_board_label = False
        else:
    ...
    def _compute_positions(self):
        ...
        self.positions = {
            i: (x - min_x + self.margin, y - min_y + self.margin)
            for i, x, y in coords
        }
    ...
    def _draw_static_board(self):
        if self.enable_injected_bugs and self._bug_skip_next_canvas_clear:
            self._bug_skip_next_canvas_clear = False
        else:
    ...
    def _redraw_stones(self):
        ...
        for index, (cx, cy) in self.positions.items():
            ...
            if (
                self.enable_injected_bugs
                and not skipped_one_stone
                and self.board_size >= 6
                and self.game.move_count() >= 18
                and self.fuzz_redraws % 5 == 0
            ):
                skipped_one_stone = True
                continue
    ...
    def _refresh_status(self, event_message=""):
        ...
        if self.game.is_done_game():
            if self.game.winner_player() == 0:
                ...
                if (
                    self.enable_injected_bugs
                    and self.game.win_structure() in (2, 3)
                    and self.game.move_count() >= 10
                ):
                    turn_text = f"Turno: {'Bianco' if self.game.current_player() == 1 else 'Nero'}"
                else:
        ...
        self.status_var.set(
            f"{turn_text} | {event_message}" if event_message else turn_text
        )

    def _draw_fake_stone_for_bug(self, index):

        if index not in self.positions:
            return

        cx, cy = self.positions[index]
        radius = self.hex_size * 0.62
        ...
        self.canvas.create_oval(
            ...
            outline="#990000",
            width=2.0,
            tags=("stone", "fake_stone", f"cell_{index}"),
        )
    ...
    def _on_cell_click(self, index):
        ...
        if self.game.is_done_game():
            if self.enable_injected_bugs and self.game.move_count() >= 10:
                self._draw_fake_stone_for_bug(index)
        ...
        if not self.session.can_play_move(move):
            if self.game.get_cell(index) != 0:
                self.fuzz_occupied_clicks += 1

                if (
                    self.enable_injected_bugs
                    and self.game.move_count() >= 12
                    and self.fuzz_occupied_clicks >= 2
                ):
                    current = "Bianco" if self.game.current_player() == 1 else "Nero"
                    self.status_var.set(f"Turno: {current}")
                    return

            self._refresh_status(
                "Cella occupata." if self.game.get_cell(index) != 0 else "Mossa non valida."
            )
            return

        self.fuzz_valid_clicks += 1
    ...
    def fuzz_reset_to_size(self, board_size):
        ...
        previous_board_size = self.board_size
        previous_move_count = int(self.game.move_count()) if self.game is not None else 0

        self.fuzz_resets += 1

        if self.enable_injected_bugs:
            if (
                previous_board_size is not None
                and previous_board_size >= 7
                and board_size == 3
                and previous_move_count >= 10
            ):
                self._bug_skip_next_canvas_clear = True

            if (
                previous_board_size is not None
                and previous_board_size == 8
                and board_size in (3, 4)
                and previous_move_count >= 8
            ):
                self._bug_keep_stale_board_label = True
